...
from helpers import default_if_empty
...
class NEODatabase:
    ...
    def __init__(self, neos, approaches):
        ...
        collections of NEOs and close approaches haven't yet
        been linked - that is, the`.approaches` attribute of each
        `NearEarthObject` resolves to an empty collection,
        and the `.neo` attribute of each `CloseApproach` is None.
        ...
        for index, neo in enumerate(self._neos):
            self._index_of_pdes[neo.designation] = index

        neo_keys = self._index_of_pdes.keys()

        for approach in self._approaches:
            if approach._designation in neo_keys:
                approach.neo = self._neos[
                    self._index_of_pdes[approach._designation]
                ]
                approach.neo.approaches.append(approach)

        self._idx_des_of_neo = {neo.designation: neo for neo in self._neos}
        self.idx_name_of_neo = {neo.name: neo for neo in self._neos}
    ...
    def get_neo_by_designation(self, designation):
        ...
        or `None`.
        """
        return self._idx_des_of_neo.get(designation, None)
    ...
    def get_neo_by_name(self, name):
        ...
        """
        return self.idx_name_of_neo.get(name, None)
